...
**DimCustomer (Type 2 - Slowly Changing)**
...
**Relationships**
1. FactSales → DimDate (Many-to-One)
Cross-filter: Single
Active: Yes
2. FactSales → DimCustomer (Many-to-One)
Cross-filter: Both (for customer insights)
3. FactSales → DimProduct (Many-to-One)
Cross-filter: Single
...
1. Indices: Columnstore indexes on high-cardinality columns
2. Sorting: DateKey sorted descending (recent data first)
3. Compression: Dictionary encoding on text columns
4. Relationships: 1-to-many only, no many-to-many without bridge
